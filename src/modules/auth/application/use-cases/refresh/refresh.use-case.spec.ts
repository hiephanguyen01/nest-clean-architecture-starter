...
class SequenceIds implements IdGenerator {
  ...
  private readonly values = [
    '6cc85d90-6321-4d11-b086-d24afdc7627a',
    '7dd96ea1-7432-4e22-a197-e35bfed8738b',
  ];
  next(): string {
...
class FakeTokens implements TokenService {
  async issueAccessToken(input: { userId: string; role: string }): Promise<IssuedToken> {
    return {
      token: `access:${input.userId}:${input.role}`,
  ...
  async issueRefreshToken(input: { userId: string; sessionId: string }): Promise<IssuedToken> {
    return {
      token: `refresh:${input.userId}:${input.sessionId}`,
      expiresAt: new Date('2026-09-12T00:00:00.000Z'),
    };
  }
  async verifyAccessToken(_token: string): Promise<AccessTokenClaims> {
...
describe('RefreshUseCase', () => {
  it('rotates a refresh token exactly once', async () => {
    ...
    await expect(useCase.execute({ refreshToken: currentToken })).rejects.toThrow(
