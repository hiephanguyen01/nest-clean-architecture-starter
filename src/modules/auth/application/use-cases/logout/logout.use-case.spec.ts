...
class Sessions extends RefreshSessionRepository {
  revoked = false;
  constructor(private readonly session: RefreshSession) { super(); }
  async create(_session: RefreshSession): Promise<void> {}
  async findById(id: string): Promise<RefreshSession | null> { return id === this.session.id ? this.session : null; }
  async rotate(_input: RotateRefreshSessionInput): Promise<boolean> { return false; }
...
const tokens: TokenService = {
  async issueAccessToken(): Promise<IssuedToken> { throw new Error('unused'); },
  async issueRefreshToken(): Promise<IssuedToken> { throw new Error('unused'); },
  async verifyAccessToken(): Promise<AccessTokenClaims> { throw new Error('unused'); },
  async verifyRefreshToken(): Promise<RefreshTokenClaims> {
    return { userId: '9d4fa61e-d2cf-4c58-8dbd-dce4220df511', sessionId: '6cc85d90-6321-4d11-b086-d24afdc7627a' };
  },
};
...
describe('LogoutUseCase', () => {
  it('revokes the refresh session after verifying the token hash', async () => {
    ...
    const useCase = new LogoutUseCase(tokens, hasher, sessions, { now: () => new Date('2026-09-05T01:00:00.000Z') });
